...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [CommonModule,ReactiveFormsModule, RouterModule, RouterLink, RegisterComponent],
...
export class LoginComponent {
 ...
 errMsg:String=""
  loginForm = new FormGroup({
      ...
      uPass: new FormControl("", [Validators.required,Validators.pattern("^.*(?=.{8,})(?=.*[a-zA-Z])(?=.*\d)(?=.*[!#$%&?]).*$")])
    });
    onSubmit(){
